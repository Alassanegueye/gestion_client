...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        echo "La réservation a été approuvée avec succès.";
    } else {
        echo "Erreur lors de l'approbation de la réservation: " . $conn->error;
    }

    $stmt->close();
}

$sql = "SELECT id, customer_name, reservation_date FROM reservations WHERE status='pending'";
...
<body>
    ...
    <main>
        <h1>Approuver une réservation</h1>
        <form method="POST" action="">
            ...
            <select id="reservationId" name="reservationId" required>
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<option value=''>Aucune réservation en attente</option>";
                }
                ?>
            </select>
